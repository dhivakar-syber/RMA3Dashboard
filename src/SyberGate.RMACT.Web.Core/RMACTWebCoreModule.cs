...
using SyberGate.RMACT.Startup;
using SyberGate.RMACT.Web.Authentication.JwtBearer;
using SyberGate.RMACT.Web.Authentication.TwoFactor;
using SyberGate.RMACT.Web.Chat.SignalR;
using SyberGate.RMACT.Web.Configuration;
using SyberGate.RMACT.Web.DashboardCustomization;

namespace SyberGate.RMACT.Web
{
    [DependsOn(
        typeof(RMACTApplicationModule),
        typeof(RMACTEntityFrameworkCoreModule),
        typeof(AbpAspNetZeroCoreWebModule),
        typeof(AbpAspNetCoreSignalRModule),
        typeof(RMACTGraphQLModule),
        typeof(AbpRedisCacheModule), //AbpRedisCacheModule dependency (and Abp.RedisCache nuget package) can be removed if not using Redis cache
        typeof(AbpHangfireAspNetCoreModule) //AbpHangfireModule dependency (and Abp.Hangfire.AspNetCore nuget package) can be removed if not using Hangfire
    )]
    public class RMACTWebCoreModule : AbpModule
    {
        private readonly IWebHostEnvironment _env;
        private readonly IConfigurationRoot _appConfiguration;

        public RMACTWebCoreModule(IWebHostEnvironment env)
        {
            _env = env;
        ...
        public override void PreInitialize()
        {
            ...
            Configuration.Modules.AbpAspNetCore()
                .CreateControllersForAppServices(
                    typeof(RMACTApplicationModule).GetAssembly()
                );
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(RMACTWebCoreModule).GetAssembly());
        }
        ...
        public override void PostInitialize()
        {
            ...
            IocManager.Resolve<ApplicationPartManager>()
                .AddApplicationPartsIfNotAddedBefore(typeof(RMACTWebCoreModule).Assembly);
        }
